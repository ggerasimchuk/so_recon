...
Every verification suite in this tree does the same three things with what a diagnostic hands
back: it rebuilds the fixture's case through the PRODUCTION contract (`CaseBundle`,
`FluidSpec`, `WellSpec`, `ControlSegment`, `validate_case`), it publishes the native
extraction through the PRODUCTION publisher (`publish_forward_result`), and it reads the
record back with every declared digest, unit, shape and physical range re-proved. That
...
It lived under `tests/integration` while Tasks 9 and 10 were its only callers. Task 12's
`verify-physics` command runs the same diagnostics from the command line and publishes the
same results, so the sequence is production code now and the test helper delegates to it —
one implementation, exercised by both. Nothing here asserts a STATUS: a fixture that is meant
to come back `CONTROL_INFEASIBLE` is as legitimate as one that completes, and deciding which
...
def build_case(
    ...
    raw_shape = [int(n) for n in case_json["grid"]["shape"]]
    raw_extent = [float(e) for e in case_json["grid"]["extent_m"]]
    if len(raw_shape) != 3 or len(raw_extent) != 3:
        raise ValueError(
            f"{label}: a case grid is three-dimensional, got shape {raw_shape} and extent "
            f"{raw_extent}"
        )
    shape: tuple[int, int, int] = (raw_shape[0], raw_shape[1], raw_shape[2])
    extent: tuple[float, float, float] = (raw_extent[0], raw_extent[1], raw_extent[2])
...
def diagnostic_solver_evidence(fixture: dict[str, Any]) -> dict[str, Any]:
    """Native diagnostic options are not a worker SolverConfig with invented defaults."""
    options = fixture.get("solver_options")
    return {
        "schema_version": "e01-diagnostic-solver-1",
        "options_available": options is not None,
        "solver_options": options,
        "note": (
            "Unlisted settings use the pinned native solver defaults; no worker defaults inferred."
        ),
    }
...
def publish_fixture(
    ...
    *,
    ...
    solver_evidence = diagnostic_solver_evidence(fixture)
    solver_path = paths.artifacts / "solver" / f"diagnostic-{sha256_json(solver_evidence)}.json"
    solver_path.parent.mkdir(parents=True, exist_ok=True)
    if not solver_path.exists():
        solver_path.write_text(
            json.dumps(solver_evidence, sort_keys=True, allow_nan=False), encoding="utf-8"
        )
    job = JobDescriptor(
