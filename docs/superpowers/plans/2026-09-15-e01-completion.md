...
The first registered gate on `f4876e8` passed P1 (including the reference pair and all five worlds), but P0 and BO were refused by the measured memory guard during worker work. Those attempts remain published. After the user freed memory, a full repeat was selected. Before that repeat, the 48-grid job was explicitly bound to `five_spot_coarse_sensitivity` in the plan: otherwise resume could drop the negative coarse diagnostic while carrying the reference PASS. Three real-plan regressions reproduced and closed that edge. Physical inputs and tolerance values did not change.

## Final registered acceptance

`E01_SUITES='bo p0 p1' bash scripts/e01_gate.sh` passed on clean code commit `45d273f2bdc12cf0957336e24b7571704b3592c6` (2026-09-15, completed 08:45:18 UTC). All three suites returned exit 0. The gate verified 797 regression tests, lint/format/types, both E00 smoke paths, and 21 published-artifact validators with no skips; frozen inputs stayed unchanged. The stage is `PASS_WITH_LIMITATIONS`, with no failed or unrun mandatory checks under the amended reference-grid scope. The coarse 16/48 FAIL is still a visible diagnostic; it cannot disappear through resume. Earlier resource refusals are retained and did not require changing resource limits.
