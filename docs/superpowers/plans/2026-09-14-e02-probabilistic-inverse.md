...
Original prior is full Gaussian: account omitted |z|>7 mass `2*norm.sf(7)`, not silently renormalize it to a different prior. Because all bin likelihood factors≤1, unnormalized omitted evidence≤tail mass; require tail_mass / estimated_Z_lower <1e-6. Estimated_Z_lower uses quadrature refinement error margin. Successive grids require posterior mean change<.02 prior sd, q05/q50/q95 change<.03, relative Z change<.02. If sharp likelihood fails, reference status `UNRESOLVED_REFERENCE`, no ground-truth posterior claim.
  ...
  **Diagnostic extension 15.09.2026 (явно разрешён пользователем):** физический experiment, truth, observations, prior, likelihood, support `[-7,7]` и четыре acceptance threshold не меняются. Отдельный protocol `e02-reduced-reference-diagnostic-v1` исправляет только численное определение continuous-z quantile: вместо inverse CDF дискретных node weights интегрировать неотрицательную piecewise-linear интерполяцию unnormalized posterior density на каждом trapezoid interval и аналитически обратить её quadratic segment CDF. Evidence остаётся обычным nested trapezoid integral; mean/variance сохраняют прежние explicit node-weight formulas для сопоставимости. Старые schema v1/v2 и их point-node quantiles неизменяемы; новый artifact schema/version обязан хранить `quantile_method`, legacy point quantiles и новые continuous quantiles. Аналитический unit test: density `f(z)=2z` на `[0,1]` имеет quantiles `sqrt(q)`; uniform density даёт quantiles `q` даже на двух узлах. На уже рассчитанных данных diagnostic recomputation 65→129 даёт continuous-quantile changes `0.0201417/0.00120406/0.0217895`, все `<0.03`; evidence change остаётся `0.0343119`, поэтому reference всё ещё не принят без нового F.

  Разрешённая nested лестница: 257, затем stop-on-convergence; при FAIL — 513, 1025 и максимум 2049. Каждый уровень сравнивается только с непосредственным coarse parent под тем же diagnostic protocol; все четыре исходных strict inequality сохраняются. Refinements выше P0 capacity используют один approved `P1_LOOP` worker/session на уровень с inherited ledger; profile transition записывается, cumulative attempts/cost не сбрасываются. Если 2049 не сходится, dependent native jobs остаются закрыты и требуется новый numerical-method review; thresholds, observations и design не менять по результату. На первом CONVERGED уровне дальнейшие reference levels не запускать и открыть пункты 11.5/11.6, затем 13.5 в исходном порядке.

  **Diagnostic result checkpoint 15.09.2026:** commit `276fec3` реализовал schema `e02-reduced-reference-3`, continuous piecewise-linear density CDF и неизменяемое сохранение legacy point-node quantiles. Nested 257-node run под `P1_LOOP` переиспользовал 129 родительских узлов и добавил 128 midpoint forwards. Сравнение 129→257 прошло все исходные критерии: mean change `2.34756e-05 < .02`, max continuous-quantile change `0.00873208 < .03`, relative evidence change `1.76552e-05 < .02`, tail/evidence lower `5.94762e-09 < 1e-6`. Статус `CONVERGED`; authority — `artifacts/runs/20260915T143601Z-e02-reduced-reference-6c2604d7/reduced_reference.json`. Cumulative chain: 258 forwards, `345.213 s` wall, `935.198 s` CPU, `44,577,887` bytes. По stop-on-convergence уровни 513/1025/2049 не запускать. Reference stop-gate снят; пункты 11.5/11.6 открыты, но общий E02 остаётся FAIL до их выполнения и native matrix.
- [ ] **11.5 Native SMC reference comparison:** two seeds 11/12 at32 and64, every run separately budgeted; same prior/L/U/support and output dates. Posterior mean differs from converged quadrature by ≤max(.05 prior sd,3 across-start MC SE), quantiles≤max(.08,3 MC SE), report evidence/modes/beta/unique ancestors. beta1 alone not pass; reference grid states produce independent truth/reference So distribution on same evaluator support. Native attempts max64 in P0: first reference stage 1 truth+33 nodes=34, retries may consume remainder; SMC physical comparisons use P1 limits if needed, not an unannounced larger P0.
  ...
  **Native SMC checkpoint 15.09.2026:** implementation commits `374d8f8` and `aa48305` added the registered 1D density/schema, separate reduced F/L caches, checkpoint/ledger/posterior publication and preserved the accepted E01 physics tree byte-for-byte. All four required native runs reached beta one with no failed forward. N32 seeds 11/12 passed the paired reference arithmetic. N64 seeds 11/12 failed only Q05: pooled `0.3954874` versus reference `0.4795004`, difference `0.0840130` against unchanged tolerance `0.08` (`3 MC SE=0.0032709`). Mean, Q50 and Q95 pass. Immutable verdict: `artifacts/runs/20260915T153008Z-e02-reduced-smc-comparison-76dbc673/reduced_smc_comparison.json`, status `FAIL`. Beta one alone was not promoted to convergence.

  **SMC tail diagnostic extension 15.09.2026 (явно разрешён пользователем до запуска):** protocol `e02-reduced-smc-diagnostic-n128-v1` keeps the exact same design, truth, observations, prior, likelihood, output dates, SMC kernel law/configuration, seeds 11/12, reference and original mean/quantile thresholds. Add exactly the paired N128 starts, each in its own `P1_LOOP` ledger; exact forward/likelihood cache hits from the same seeded prefixes may be reused after immutable verification, but costs are reported as new and cache-hit evaluations separately. Central estimates and across-start MC SE use the same frozen comparator as N32/N64; do not replace empirical weighted quantiles after seeing results. If N128 passes, classify Task 11.5 `PASS_WITH_LIMITATIONS` (the required N64 matrix remains an immutable marginal FAIL, while doubled-N resolves it) and proceed to 11.6. If N128 fails, keep 11.5 failed and perform a sampler/numerical-method review before authorizing any further particle ladder; do not loosen thresholds or reinterpret the reference.

  **SMC tail diagnostic result 15.09.2026:** N128 seeds 11/12 completed at beta one with 1,104/1,088 new forwards, zero failed forwards and 64/70 unique ancestors. Paired mean `0.6917704` differs from quadrature by `0.0040599 < 0.05`; Q05/Q50/Q95 `[0.5174891, 0.6738848, 0.9640040]` differ by `[0.0379887, 0.0023676, 0.0129610]`, all `<0.08` (and all computed by the unchanged empirical weighted quantile). Authority: `artifacts/runs/20260915T161734Z-e02-reduced-smc-comparison-151116e0/reduced_smc_comparison.json`, status `PASS`, protocol `e02-reduced-smc-diagnostic-n128-v1`. Task 11.5 status is `PASS_WITH_LIMITATIONS`; original N64 FAIL remains an immutable limitation rather than being overwritten.
- [ ] **11.6 Noise recovery/calibration:** fixed physical prediction bank reuse for 200 independent recursive noisy observations, fresh noise seeds 8000–8199. For sigma/rho recovery use 2D deterministic quadrature over the declared transformed-prior coordinates, with grid refinement, no new F for noise-only changes; true sigma/rho drawn from their declared prior. Check repeated coverage and boundary-bin frequencies, record Wilson intervals and rho/sigma correlation. Include missing month3, reset7, dry observed masks, exact0/1 realizations, and Task5 date/bias mixture on saved report states. 200 *noise* replicates are conditional on this F, not 200 independent geological worlds; full system repeated-physical-world coverage deferred to E03/E11 and stated as limitation.
  ...
  **Noise recovery numerical protocol 15.09.2026 (registered before the 200-run artifact):** use tensor Gauss-Hermite quadrature directly under the independent standard-normal `z_sigma,z_rho` prior, coarse 33×33 then fine 65×65, so no finite latent truncation or physical Jacobian is introduced. Per replicate require strict fine/coarse changes: sigma mean `<0.0015`, rho mean `<0.02`, sigma Q05/Q95 `<0.004`, rho Q05/Q95 `<0.04`, relative evidence `<0.02`; if any replicate fails, refine the whole suite to 129×129 before a verdict. Fine marginal 90% intervals use the frozen midpoint-CDF interpolation of quadrature masses. Sigma and rho coverage each pass only when the 95% Wilson interval from all 200 replicates contains nominal `0.90`. Report endpoint-bin frequencies and Wilson intervals without fitting to them. Separate deterministic contract checks must show finite exact-0/exact-1 bounded-bin likelihood, preserve missing month3 and reset7, refuse an observed dry prediction, and evaluate one shared-date/two-interval log factor with a shared bias on exact saved truth report states. Protocol ID/schema `e02-noise-recovery-1`; any Julia forward during this suite is a protocol failure.

  **Noise recovery result 15.09.2026:** commit `a37b838` implemented the registered two-dimensional quadrature and checked its selected-bin vectorized likelihood against the public recursive scorer to `1e-12`. All 200 seeds 8000–8199 converged at 65×65; the largest fine/coarse changes were sigma mean `0.000116394`, rho mean `0.00106335`, sigma tail quantile `0.00390789`, rho tail quantile `0.0291831`, and relative evidence `0.00525163`, each below its unchanged threshold. Marginal 90% coverage was sigma `0.92` with Wilson-95 `[0.874011,0.950159]` and rho `0.89` with Wilson-95 `[0.839073,0.926228]`; both intervals contain `0.90`. The deterministic endpoint/missing/reset/dry/date/shared-bias contract passed on the exact saved truth states. Authority: `artifacts/runs/20260915T162834Z-e02-noise-recovery-33b4da79/noise_recovery.json`, status `PASS`; `new_physical_forwards=0`. This is conditional-on-one-F calibration, not repeated-world calibration. Task 11.6 is complete and Task 13.5 is open.
...
**Resource checkpoint 15.09.2026:** after registering T2-v2 on commit `440db68`, the mandatory E01 refresh ran the full `1255 passed` unit regression, lint/format/mypy, legacy smoke, Julia smoke and analytic/refinement groups, but the ledger refused the warm-worker P0/P1 groups because the measured hard cap varied below the resident worker (`~0.84–1.83 GiB` cap versus `~1.30–1.91 GiB` process tree). Authorities: `artifacts/runs/20260915T170052Z-verify-physics-dc6e8e04`, `artifacts/runs/20260915T170402Z-verify-physics-dc6e8e04`, and fail-closed report `artifacts/runs/20260915T171445Z-e01-report-659c0dd3/E01.json`. This is `RESOURCE_FAILURE`, not physics evidence; all E02 native execution remains blocked until a fresh accepted E01 report is produced without reducing the 6 GiB host reserve or other frozen resource limits. Independent code/tests may continue.
...
T2: two layers with equal thickness, equal base K=80mD/phi relation, identical horizontal design, `kz/kx=1e-4` — объявленный low-crossflow case на существующем permeability input E01; both layers completed in same wells, layer-symmetric controls without P2 lower-layer closure and common pressure reference. Same finite cosine family in each layer, G logk supports observe equal-weight layer-average **log k** (A symmetric); conditional Gaussian law preserves exchange symmetry. Contrasting layer coefficients arise from residual/latent draw, not hand-painted So. s fixed; report swapped-layer physical pair through F: преобразовать theta→physical coefficients a, переставить два layer blocks, решить `z=Q.T@solve(chol,a_swapped-mean)`, затем разложить обратно v/z_perp с теми же nuisance. Перестановка первых/последних whitened coordinates сама по себе не является перестановкой слоёв. Gravity/compressibility/crossflow/well constraints могут нарушать точную симметрию, поэтому сравниваются actual outputs. Pre-register acceptable ambiguity evidence mean absolute fw difference≤.01 AND at least one layer-zone |So_a-So_b|≥.05 at month36; otherwise `AMBIGUITY_NOT_DEMONSTRATED`, revise design under new ID. Не добавлять несуществующие face-multiplier/gravity API ради этого fixture.
  ...
  **T2 control-feasibility checkpoint 15.09.2026:** first native truth for immutable `e02-t2-v1` ended `CONTROL_INFEASIBLE` before observations or pair scoring: under the rate targets, I1 hit the 30 MPa limit at roughly 8–12 m³_sc/day and P1 hit the 5 MPa limit at roughly 4–5 m³_sc/day. Authority: `artifacts/runs/20260915T165158Z-e02-physical-setup-71160d9a/run.json`, status `FAIL`. This is a technical design failure, not ambiguity evidence, and no SMC is allowed on v1.

  Registered replacement `e02-t2-v2` changes only the control law: all four wells use fixed BHP throughout the same 36-month calendar, 30 MPa for injectors and 5 MPa for producers, with both layer connections open and no rate-limit switching. Geometry, 80 mD symmetric layers, `kz/kx=1e-4`, truth seed143, G construction, latent law, physical layer-swap, report support and ambiguity thresholds remain unchanged. The new design/experiment ID forces regeneration of truth, G, history and pair evidence; no v1 forward or observation is reused. If v2 truth, balance, finite-state or ambiguity checks fail, record the failure and review the fixture again rather than changing acceptance thresholds.
...
Initial pressure — тот же oil-connected hydrostatic initializer при 15MPa datum, допустимый начальный transient S0 с пространственно неоднородным Sw. Сохранять interpretation `synthetic_nonvirgin_initial_state`, не equilibrium/virgin Soi claim. Native closed preflight проверяет finite states, Sw/So bounds, component mass conservation и воспроизводимость transient, **не** отсутствие движения: при общей oil pressure вода может перераспределяться. Все starts используют этот declared initializer. Если transient/reference fails, исправить физическую постановку до inverse. No G supports in eastern quadrant; неизвестное So0 необходимо, поскольку при точно известной Sw=.2 и отсутствии вытеснения узкий posterior So=.8 сам по себе не был бы ложной уверенностью.
  ...
  **T4 closed-transient protocol registered before execution 15.09.2026:** build a one-month case from the exact T4 rock/fluids/initial arrays with every surface control disabled and every perforation masked. Execute it twice as independent native jobs. Both jobs must be COMPLETE, finite and bounded and pass the unchanged physical truth balance limits (`cumulative≤1e-3`, median step `≤1e-4`); initial output must reproduce declared pressure/So within `1e-10` relative/absolute. The two trajectories must agree within relative pressure and absolute So `1e-12`. Non-zero initial-to-month-one drift is reported, not failed. Any failed check blocks T4 truth/history/pair/SMC without widening these limits.

  The first post-preflight setup attempt was refused before native work because the T2-v2 implementation had accidentally added a `control_protocol` metadata key to the existing T4-v1 design payload, conflicting with its immutable truth path. This was a version-identity defect, not a T4 result. The key is restricted to the new T2-v2 ID and a regression test pins the unchanged T4-v1 payload; T4 keeps its original ID and bytes because neither its physics nor metadata contract changed.

  **Native setup result 15.09.2026:** T2-v2 authority `artifacts/runs/20260915T174621Z-e02-physical-setup-01a5f275/physical_experiment.json` is PASS. Truth cumulative/median-step balance errors are `9.63298e-7`/`9.23142e-10`; the physical layer-swap pair has mean absolute fw gap `1.61813e-4≤.01` and maximum layer-zone So gap `.159121≥.05`, hence `AMBIGUITY_DEMONSTRATED`. T4 authority `artifacts/runs/20260915T180505Z-e02-physical-setup-e6c8088e/physical_experiment.json` is PASS. Both closed replays pass balances and reproduce each other exactly at the published precision; initial pressure/So gaps are zero, while the reported one-month transient is relative pressure `1.00947e-4` and So `8.19911e-5`. T4 truth cumulative/median-step balance errors are `4.25931e-6`/`1.78915e-9`; its remote pair has mean fw gap `2.13587e-4` and maximum remote-zone So gap `.171021`, hence `AMBIGUITY_DEMONSTRATED`. Final E01 authority after the T4 identity repair is `artifacts/runs/20260915T180437Z-e01-report-df99ba23/E01.json`, accepted commit `033a956`, 13 mandatory OW checks PASS.
...
- [ ] **14.4 Forecast before native inverse.** Each ordinary run bound, without cache/rejections/retries: `N_initial + N * moves_per_level * max_beta_steps`; N32→1568, N64→3136, **so N64 at24 levels cannot fit 2000 forwards in worst case**. Keep mathematical settings and return budget stop; do not claim guaranteed completion. Budget predictor separately shows measured expected accepted proposals/cache hits and worst case; use initial N32 and observed actual beta levels to authorize N64 session, or checkpoint/resume under explicit run intent. Do not silently reduce transitions/convergence just to claim PASS.
  ...
  **Resume implementation checkpoint 15.09.2026:** a `BudgetStop` raised while evaluating the already-persisted pending proposal is translated to SMC `INCOMPLETE_BUDGET`, not `EVALUATION_FAILURE`; the exact phase/cursor/RNG/pending proposal and the complete initial prior ensemble remain in the immutable checkpoint. A resume validates proposal/target/basis/config/component hashes, opens a new P1 ledger inheriting the prior ledger, and continues into a new run directory. Native integration now probes actual host resources; the former synthetic 64/32 GiB snapshot was removed because it bypassed the frozen 6 GiB host reserve. Numerical/runtime target failures retain `EVALUATION_FAILURE`.
